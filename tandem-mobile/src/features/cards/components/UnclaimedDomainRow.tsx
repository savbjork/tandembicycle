...
export const UnclaimedDomainRow: React.FC<UnclaimedDomainRowProps> = ({ card, onClaim }) => (
  <View className="bg-cream rounded-xl px-4 py-3.5 mb-2 border border-border border-dashed flex-row justify-between items-center">
    <Text className="text-base font-medium text-text-secondary">{card.name}</Text>
    <TouchableOpacity className="bg-accent-claim px-4 py-2 rounded-lg" onPress={onClaim}>
