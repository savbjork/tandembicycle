...
export const InboxScreen: React.FC = () => {
  ...
  const { dropZoneItems, addDropZoneItem, updateDropZoneItemStatus, removeDropZoneItem } =
    useDataStore();
  ...
  const receivedItems = useMemo(
    () =>
      dropZoneItems.filter(
        (item: DropZoneItem) => item.receiver === currentUser && item.status === 'pending'
      ),
        ...
        {sentItems.length > 0 && (
          <View className="mb-6">
            ...
            {sentItems.map((item: DropZoneItem) => (
              <DropItemCard key={item.id} item={item} onDelete={() => handleDelete(item.id)} />
            ))}
          </View>
        )}
        <View className="h-10" />
...
const DropItemCard: React.FC<DropItemCardProps> = ({ item, onCheckmark, onDelete }) => (
  <View className="bg-surface rounded-xl p-4 mb-2 border border-border-light shadow-sm">
    ...
    <View className="flex-row items-center gap-2">
      ...
      {item.status === 'archived' && <Badge variant="success" size="sm" label="Archived" />}
    </View>
  </View>
);
