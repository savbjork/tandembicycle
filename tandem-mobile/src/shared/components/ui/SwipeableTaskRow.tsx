...
export const TaskRow: React.FC<TaskRowProps> = ({
  ...
  if (isCompact) {
    return (
      ...
      >
        ...
        {!hideDueDate && (
          <Text
            className={`text-[11px] ${overdue ? 'text-error-500 font-semibold' : 'text-text-muted'}`}
          >
  ...
  if (isBoard) {
    return (
      <TouchableOpacity
        className={`${hideBackground ? '' : 'bg-surface shadow-sm ' + (overdue ? 'border border-error-200' : '')} rounded-lg px-3.5 py-3 mb-2`}
        onPress={handlePress}
        activeOpacity={0.7}
      >
        <View className="flex-row items-center gap-2">
          ...
          <Text
            className={`text-[14px] font-medium flex-1 ${isDone ? 'text-text-muted line-through' : overdue ? 'text-error-600' : 'text-text'}`}
          >
            {task.name}
          </Text>
        </View>
        <View className="flex-row items-center justify-between mt-1.5 ml-6">
          ...
          {!hideDueDate && (
            <Text
              className={`text-xs ${overdue ? 'text-error-500 font-semibold' : 'text-text-secondary'}`}
            >
  ...
  return (
    <TouchableOpacity
      className={`${hideBackground ? '' : 'bg-surface shadow-sm ' + (overdue ? 'border border-error-200' : '')} ${hideBackground ? '' : hideCardName ? 'mb-1.5' : 'mb-3'} ${hideCardName ? 'py-1.5' : 'py-3.5'} rounded-xl px-4`}
      onPress={handlePress}
      activeOpacity={0.7}
    >
      <View className={`flex-row items-center ${hideCardName ? 'gap-2.5' : 'gap-3'}`}>
        ...
        <View className="flex-1">
          <Text
            className={`${hideCardName ? 'text-[14px]' : 'text-[15px]'} font-medium ${isDone ? 'text-text-muted line-through' : overdue ? 'text-error-600' : 'text-text'}`}
          >
            {task.name}
          </Text>
          {!hideCardName && (
        ...
        <View className="items-end flex-row items-center gap-3">
          ...
          {showOwnerIcon && (
            <View
              className={`w-8 h-8 rounded-full items-center justify-center ${task.owner === currentUser ? 'bg-primary-600' : 'bg-secondary-600'}`}
            >
              <Text className="text-white text-[12px] font-bold">{task.owner.charAt(0)}</Text>
            </View>
          )}
        </View>
      </View>
